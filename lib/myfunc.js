import { jidNormalizedUser, getContentType, areJidsSameUser, downloadContentFromMessage } from './baileys.js'
import fs from 'fs'
...
export const smsg = (conn, m, store) => {
    ...
    // reaksi emoji ke pesan (dipakai sebagian plugin plugin hasil adaptasi)
    m.react = async (emoji) => {
        try {
            return await conn.sendMessage(m.chat, { react: { text: emoji, key: m.key } })
        } catch {
            return null
        }
    }

    // Unduh media pesan — plugin adaptasi base lain memakai m.download() /
    // m.quoted.download(); di base ini tidak ada, jadi disediakan di sini.
    const downloadMedia = async (msg, mtype) => {
        const jenis = String(mtype || '').replace(/Message$/, '')
        if (!msg || !jenis) throw new Error('Pesan ini tidak memiliki media')
        const stream = await downloadContentFromMessage(msg, jenis)
        let buffer = Buffer.from([])
        for await (const chunk of stream) buffer = Buffer.concat([buffer, chunk])
        return buffer
    }

    m.download = async () => downloadMedia(m.msg, m.mtype)
    if (m.quoted?.msg) {
        m.quoted.download = async () => downloadMedia(m.quoted.msg, m.quoted.mtype)
    }
