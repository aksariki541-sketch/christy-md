...
let handler = async (m, { conn, text, args, usedPrefix, command }) => {
	// Kompat: fork Baileys ini tidak menyediakan conn.sendButton.
	// Bungkus menjadi sendMessage + interactiveButtons agar tombol tetap terkirim.
	if (conn && typeof conn.sendButton !== 'function') {
		conn.sendButton = (chat, content = {}, options = {}) => {
			const { buttons, ...rest } = content || {};
			return conn.sendMessage(
				chat,
				{ ...rest, ...(buttons ? { interactiveButtons: buttons } : {}) },
				options
			);
		};
	}
