...
// Asal       : plugins/anime/random.js (paket plugin Drive)
...
if (command == 'Christy') {
let res = await (await fetch(`https://raw.githubusercontent.com/KazukoGans/database/main/anime/Christy.json`)).json()
let cita = res[Math.floor(Math.random() * res.length)]
conn.sendFile(m.chat, cita, 'result.jpg', `Nihh ${command} nya @${m.sender.split('@')[0]}`, m) 
}
if (command == 'minato') {
