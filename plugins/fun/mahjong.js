...
// Asal       : plugins/game/mahjong.js (paket plugin Drive)
// Penyesuaian: handler.command jadi array, properti handler.* yang tidak didukung dibuang,
//              kategori/deskripsi ditambahkan, branding base lama dibersihkan.
// Command    : .mahjong, .mjong, .mj, .kyoko

/**
 * KYOKO MAHJONG — fixed for Christy MD
 * - signature disamakan dengan game-dino (yang work)
 * - payload + botResponseId pakai UUID yang sama
 * - struktur kirim = pola tetris/dino
 */
...
const MJ_HTML = `<style>
*{box-sizing:border-box;margin:0;padding:0;font-family:Arial,Helvetica,sans-serif;-webkit-tap-highlight-color:transparent;user-select:none;-webkit-user-select:none;-webkit-touch-callout:none}
html,body{width:100%}
...
async function sendMahjong(conn, chat) {
...
const handler = async (m, { conn }) => {
 try {
 await m.react('🀄').catch(() => {})
 await sendMahjong(conn, m.chat)
 await m.react('✅').catch(() => {})
 } catch (e) {
 console.error('[MAHJONG]', e)
 await m.react('❌').catch(() => {})
 await m.reply('❌ Gagal buka mahjong: ' + (e?.message || e))
 }
}

handler.command = ['mahjong', 'mjong', 'mj', 'kyoko']

handler.category = 'Fun'
handler.description = 'Mahjong'
