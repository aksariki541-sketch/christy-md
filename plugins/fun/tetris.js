...
// Asal       : plugins/game/tetris.js (paket plugin Drive)
// Penyesuaian: handler.command jadi array, properti handler.* yang tidak didukung dibuang,
//              kategori/deskripsi ditambahkan, branding base lama dibersihkan.
// Command    : .tetris, .tetrisgame, .gametetris

/**
 * =============================================================
 * NAME : TETRIS MINI
 * FIXED : Christy MD
 * NOTE : BGM base64 700KB dihapus (bikin WA gagal load)
 * payload + verification metadata = pola game-dino
 * =============================================================
 */
...
<body style="margin:0;background:#0d0e15;font-family:Arial,Helvetica,sans-serif;color:#eee;touch-action:manipulation;cursor:pointer">
<div style="width:100%;max-width:440px;margin:auto;padding:12px;box-sizing:border-box">
<div style="background:rgba(255,255,255,.05);border:2px solid rgba(108,92,231,.4);border-radius:18px;overflow:hidden;box-shadow:0 10px 40px rgba(0,0,0,.6)">
 ...
 <!-- Audio Element media/menu.mp3 -->
 <!-- BGM dihapus: base64 700KB bikin WA gagal load -->
<audio id="tetrisBgm" loop preload="none"></audio>
 ...
 <button id="bgmBtn" style="grid-column:span 2;background:rgba(255,255,255,.12);border:1px solid rgba(255,255,255,.25);color:#fff;margin-top:2px;padding:11px;font-size:10px;font-weight:bold;border-radius:8px;cursor:pointer" type="button">🎵 MUSIK BGM: OFF</button>
...
try { if (!bgmAudio.src || bgmAudio.src === window.location.href) { bgmBtn.style.opacity = .45; bgmBtn.title = 'BGM offline'; } } catch(e) {}
...
function arenaSweep() {
 ...
 if (rowCount > 0) {
 // Standard Arcade Tetris Scoring System
...
// Authentic Classic Arcade Colors
const colors = [
 null,
 '#a000f0', // T - Purple
 '#f0f000', // O - Yellow
...
function playerDrop() {
 ...
 player.pos.y--;
 merge(arena, player);
 player.score += 10; // Soft drop bonus points
...
function playerReset() {
 ...
 gameOver = true;
 bgmAudio.pause();
 if (bgmBtn) bgmBtn.textContent = '🎵 MUSIK BGM: OFF';
...
async function sendTetris(conn, chat) {
...
const handler = async (m, { conn }) => {
 try {
 await m.react('🎮').catch(() => {})
 await sendTetris(conn, m.chat)
 await m.react('✅').catch(() => {})
 } catch (e) {
 console.error('[TETRIS]', e)
 await m.react('❌').catch(() => {})
 await m.reply('⚠️ Gagal buka Tetris: ' + (e?.message || e))
 }
}

handler.command = ['tetris', 'tetrisgame', 'gametetris']
handler.category = 'Fun'
handler.description = 'Tetris'
