...
klik wa.me/${conn.user.jid.split`@`[0]}`, m.chat, {
     ...
     if (!room.pilih) this.reply(m.chat, room.p,`[S U I T  P V P]\n•Menang: ${room.poin} XP\n•Kalah: ${room.poin_lose} XP\nSilahkan Pilih\n_______________________________\n*[ GUNTING ]*\n*[ BATU ]*\n*[ KERTAS ]*`, m)
      if (!room.pilih2) this.reply(room.p2, `[S U I T  P V P]\n•Menang: ${room.poin} XP\n•Kalah: ${room.poin_lose} XP\nSilahkan Pilih\n_______________________________\n*[ GUNTING ]*\n*[ BATU ]*\n*[ KERTAS ]*`, m)
...
`.trim(), m, { contextInfo: { mentionedJid: [room.p, room.p2] } })
      if (!tie) {
        db.data.users[win == room.p ? room.p : room.p2].exp += room.poin
        db.data.users[win == room.p ? room.p2 : room.p].exp += room.poin_lose
        db.data.users[win == room.p ? room.p : room.p2].win += 1
        db.data.users[win == room.p ? room.p2 : room.p].win += 0
      }
      delete this.suit[room.id]
    }
  }
